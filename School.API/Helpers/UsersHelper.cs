...
namespace School.API.Helpers
{
    ...
    public class UsersHelper : IDisposable
    {
        ...
        public static Response CreateUserASP(UserRequest userRequest)
        {
            ...
            {
                var userManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(userContext));
                var oldUserASP = userManager.FindByEmail(userRequest.UserName);
                if (oldUserASP != null)
                {
                    return new Response
                    {
                ...
                if (result.Succeeded)
                {
                    var newUserASP = userManager.FindByEmail(userRequest.UserName);
        ...
        public static async Task PasswordRecovery(string email)
        {
            ...
            if (response.Succeeded)
            {
                ...
                var body = string.Format(@"
                    <h1>School App - Recuperación de contraseña</h1>
