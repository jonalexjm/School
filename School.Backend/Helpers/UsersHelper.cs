namespace School.Backend.Helpers
{
    ...
    public class UsersHelper : IDisposable
    {
        ...
        public static Response CreateUserASPClaim(string email, string roleName, string password, string firstName, string lastName, string photo)
        {
            ...
            {
                var userManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(userContext));
                var oldUserASP = userManager.FindByEmail(email);
                if (oldUserASP != null)
                {
                    return new Response
                    {
                ...
                if (result.Succeeded)
                {
                    var newUserASP = userManager.FindByEmail(userASP.UserName);
                    userManager.AddClaim(newUserASP.Id, new System.Security.Claims.Claim(ClaimTypes.GivenName, firstName));
                    userManager.AddClaim(newUserASP.Id, new System.Security.Claims.Claim(ClaimTypes.Name, lastName));

                   
                    if (!string.IsNullOrEmpty(photo))
                    {
                        userManager.AddClaim(newUserASP.Id, new System.Security.Claims.Claim(ClaimTypes.Uri, photo));
                    }
